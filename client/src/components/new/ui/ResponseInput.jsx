...
export const ResponseInput = ({ ref, submitAnswer, pronoun }) => {
   ...
   return (
      <div className='flex gap-1 text-xl items-center justify-center py-2'>
         <div>{pronoun}</div>
         <input ref={ref} id='response' onKeyUp={pressEnter} maxLength='25' className='bg-gray-200 px-1 py-2 max-w-40 text-[15px] text-center' type='text'/>
         <div onClick={submitAnswer} className='flex leading-3 p-1 cursor-pointer rounded-sm justify-center border h-6 w-6 border-gray-200 hover:bg-gray-100'><ArrowRightIcon /></div>
      </div>
   )
}